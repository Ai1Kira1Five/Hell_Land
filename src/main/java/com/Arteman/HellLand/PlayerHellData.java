...
public class PlayerHellData
{
	public static final int	BOOLEANS = 26, HAMMER_LAST_SMASH_TICKS = 27, FLAIL_ENTITY_ID = 28;
	...
	public static int getLastWarhammerSmashTicks(EntityPlayer player)
	{
		try
		{
			return player.getDataWatcher().getWatchableObjectInt(HAMMER_LAST_SMASH_TICKS);
		} catch (NullPointerException e)
		{
			unavailableError(player, HAMMER_LAST_SMASH_TICKS);
		}
		return 0;
	}
	...
	public static void setLastWarhammerSmashTicks(EntityPlayer player, int age)
	{
		try
		{
			player.getDataWatcher().updateObject(HAMMER_LAST_SMASH_TICKS, age);
		} catch (NullPointerException e)
		{
			unavailableError(player, HAMMER_LAST_SMASH_TICKS);
		}
